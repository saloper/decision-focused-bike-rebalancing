...
from dfbr.models.station_targets_routing import BikeStationTargetsRouting
...
def main():
    ...
    run_dir = get_path(f"experiments\\routing\\healthy_ride\\{config["experiment_name"]}\\{timestamp}")
    ...
    #Warmstart 
    #Training loop
    for epoch in range(config["training"]["warmup_epochs"]):
        epoch_train_loss = []
        pred_model.train()
        cost_head.train()
        #Training Loop
        ...
        logger.info(f"Warmup Epoch {epoch+1} Train Loss: {(sum(epoch_train_loss) / len(epoch_train_loss)):.4f}")     
