...
for s_id, station in sim.stations.items():
    ...
    forced_ret_data[s_id] = station.forced_returns
...
df_fr = pd.DataFrame(forced_ret_data).T
...
#Plot Forced Returns (Orange)
sns.heatmap(df_fr, ax=axes[2], cmap='Oranges', cbar_kws={'label': 'Forced Dumps'})
axes[2].set_title('Forced Returns', fontsize=14)
