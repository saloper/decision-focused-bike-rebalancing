...
def get_healthy_ride_trip_data(portal):

    #Get metadata for the POGOH trip dataset
    package = portal.action.package_show(id='healthyride-trip-data')
    resources = package['resources']
    ...
    for res in resources:
        #Don't download data dictionary
        if "Dictionary" in res['name']:
            continue
    ...
    if len(all_data) > 0:
        final_df = pd.concat(all_data, ignore_index=True)
        print(f"\nSuccess! Fetched {count} quarters. Total rows: {len(final_df)}")
        return final_df
    else:
        print("No data found.")
        return None
    
def download_healthy_ride_trip_data(trip_file):

    #Connect to WPRDC
    wprdc = RemoteCKAN('https://data.wprdc.org')
    trips = get_healthy_ride_trip_data(wprdc)
    trips.columns = ['TripId', 'Start Date', 'End Date', 'BikeId', 'Duration', 'Start Station Id', 'Start Station Name', 'End Station Id', 'End Station Name', 'Rider Type']
    #Clean up datatypes
    trips['Start Date'] = pd.to_datetime(trips['Start Date'], errors='coerce').dt.tz_localize('America/New_York', ambiguous=True, nonexistent='shift_forward')
    trips['End Date'] = pd.to_datetime(trips['End Date'], errors='coerce').dt.tz_localize('America/New_York', ambiguous=True, nonexistent='shift_forward')

    categorical_cols = ['Start Station Name', 'End Station Name', 'Rider Type']
    for col in categorical_cols:
        if col in trips.columns:
            trips[col] = trips[col].astype('category')
...
if __name__ == '__main__':
    ...
    download_healthy_ride_trip_data("data/raw/healthy_ride_trips.parquet")